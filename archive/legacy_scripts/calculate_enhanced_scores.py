#!/usr/bin/env python3
"""
Calculate enhanced quality scores with improved clustering and top 5 guarantee
"""
...
def main():
    """Calculate and display enhanced scores across all weeks."""
    print("=== ENHANCED QUALITY SCORES WITH IMPROVED CLUSTERING & TOP 5 GUARANTEE ===\n")
    
    # Manually extracted data from recent runs with enhanced features
    weeks_data = [
        {
            'week': 'Week 1',
            'quality_score': 71.6,
        ...
        },
        {
            ...
            'staff_efficiency': 4.1,
            'clustering_quality': 50.0,
            'total_violations': 7,
            'violations_fixed': 4,
            'ml_confidence': 1.00,
            'adaptive_score': 57.94,
            'clustering_improvement': 0.000,
            'top5_satisfied': 7,
            'top5_total': 9,
            'top5_percentage': 77.8
        }
    ]
    ...
    print("-" * 100)
    print(f"{'Week':<10} {'Quality':<8} {'Schedule':<8} {'Staff':<8} {'Cluster':<8} {'Violations':<12} {'Top5':<10} {'ML Conf':<8} {'Clust Imp':<10}")
    print(f"{'':<10} {'Score':<8} {'Quality%':<8} {'Eff%':<8} {'Quality%':<8} {'Total':<6} {'Fixed':<6} {'Sat%':<10} {'idence':<8} {'rovement':<10}")
    print("-" * 100)
    ...
    for week_data in weeks_data:
        print(f"{week_data['week']:<10} {week_data['quality_score']:<8.1f} "
              ...
              f"{week_data['violations_fixed']:<6} {week_data['top5_percentage']:<10.1f} "
              f"{week_data['ml_confidence']:<8.2f} {week_data['clustering_improvement']:<10.3f}")
    
    # Display averages
    print("\nENHANCED AVERAGE RESULTS ACROSS ALL WEEKS:")
    print("=" * 100)
    ...
    print(f"• Top 5 Preference Satisfaction: {averages['top5_percentage']:.1f}% "
          f"({'Excellent' if averages['top5_percentage'] >= 95 else 'Good' if averages['top5_percentage'] >= 80 else 'Needs Work'})")
    print(f"• Clustering System: {'Active' if averages['clustering_improvement'] > 0 else 'No improvements found'}")
    
    # Comparison with previous results
    print("\nCOMPARISON WITH PREVIOUS RESULTS:")
    print("-" * 50)
    print("Previous Average Quality Score: 69.6")
    print(f"Enhanced Average Quality Score: {averages['quality_score']:.1f}")
    improvement = averages['quality_score'] - 69.6
    print(f"Improvement: {improvement:+.1f} points")
    
    print("\nPrevious Top 5 Satisfaction: Not tracked")
    print(f"Enhanced Top 5 Satisfaction: {averages['top5_percentage']:.1f}%")
